...
pub fn _impl(ast: DefineCoreClassAst) -> syn::Result<TokenStream> {
    ...
    let parent = ast.parent.clone().unwrap_or(syn::Expr::Path(syn::ExprPath {
        attrs: Vec::new(),
    ...
    let overriding_method_ids = ast
        ...
        .map(|x| &x.id)
        .collect::<Vec<_>>();
    let overriding_method_generic_counts = ast
        .overriding_method_ids
        .iter()
        .map(|x| {
            x.generic_count
    ...
    Ok(quote! {
        ...
        impl #method_id_enum_ident {
            pub fn get_attr(&self) -> #global_crate::attrs::MethodAttr<#stdlib_header_crate::CoreTypeRef> {
                match self {
                    #(
                        Self::#overriding_method_ids => #global_crate::attr!(
            ...
            pub fn get_generic_count(&self) -> Option<#stdlib_header_serde_crate::GenericCount> {
                match self {
                    #(
                        Self::#overriding_method_ids => #overriding_method_generic_counts,
                    )*
                    #(
            ...
            pub fn get_return_type(&self) -> #stdlib_header_crate::CoreTypeRef {
                match self {
                    #(
                        Self::#overriding_method_ids => #overriding_method_return_types,
                    )*
                    #(
        ...
        pub fn #id() -> CoreTypeInfo {
            CoreTypeInfo {
                id: CoreTypeId::#id,
                kind: #stdlib_header_serde_crate::CoreTypeKind::Class,
