...
pub(crate) macro common_new_method($mt:ident $TMethodId:ident $id:ident $f:path) {
    Method::native(
        Some($mt),
        widestring::Utf16String::from_str($TMethodId::$id.get_name()),
...
fn map_field_info(
    ...
    Field::new(widestring::Utf16String::from_str(&name), attr, ty.into())
}
...
) -> impl FnOnce(&Assembly) -> TypeContainer {
    move |assembly| {
        let CoreTypeInfo {
            id: _,
            kind,
...
pub fn define_interface(
    ...
    move |assembly| {
        ...
        TypeContainer::from(Interface::new(
            NonNull::from_ref(assembly),
            widestring::Utf16String::from_str(&name),
            attr,
            generic_count
