    ...
    pub fn try_into_short(
        ...
        match self {
            ...
            Instruction_New::NewObjectOnStack {
                ...
                .and_then(|args| output.try_into_short().map(|output| (args, output)))
            {
                Some((args, output)) => Ok(Instruction_New::NewObjectOnStack {
                    ty,
                    ctor_name,
                    args,
                    output,
                }),
                None => Err(Instruction_New::NewObjectOnStack {
                    ty,
                    ctor_name,
                    args,
                    output,
            ...
            Instruction_New::NewArrayOnStack {
                ...
                Some(output) => Ok(Instruction_New::NewArrayOnStack {
                    element_type,
                    len,
                    output,
                }),
                None => Err(Instruction_New::NewArrayOnStack {
                    element_type,
                    len,
                    output,
                }),
            ...
            Instruction_New::NewDynamicArrayOnStack {
                ...
                .and_then(|len_addr| output.try_into_short().map(|output| (len_addr, output)))
            {
                Some((len_addr, output)) => Ok(Instruction_New::NewDynamicArrayOnStack {
                    element_type,
                    len_addr,
                    output,
                }),
                None => Err(Instruction_New::NewDynamicArrayOnStack {
                    element_type,
                    len_addr,
                    output,
                }),
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Instruction_New::NewObjectOnStack {
                ...
                " Stack Object {ty} {ctor_name}({}) -> {output:#x}",
                display_args(args)
            )),
            Instruction_New::NewArrayOnStack {
                element_type,
                ...
                " Stack Array[{element_type}] of length {len}({len:#x}) -> {output:#x}"
            )),
            Instruction_New::NewDynamicArrayOnStack {
                element_type,
                len_addr,
                ...
                " Stack DynamicArray[{element_type}] of length {len_addr:#x} -> {output:#x}"
            )),
        }
    }
}
